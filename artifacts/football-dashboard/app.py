...
_PAGES_WITHOUT_COMP_FILTER = {"🤖 Assistant IA", "🌍 Effectifs CM 2026", "⚽ Effectifs Clubs", "📅 Calendrier CDM 2026", "🏅 Classement ELO", "🔮 Prédictions", "🔬 Backtest V8", "📡 Cotes Betclic", "🎯 Garantie 2+", "🔮 Prédiction Buteurs", "📈 Tracking Test Edge Buteurs", "📊 Suivi des paris", "📦 Export Carrière TM"}
...
# ═══════════════════════════════════════════════════════════════════
    ...
    with tab_matches:
        ...
        from g2_engine import lambdas_buchdahl as _m1x2_lambdas
...
# ═══════════════════════════════════════════════════════════════════
    ...
    if not g2_matches:
        ...
        if st.session_state.get("_g2_bf_auto_key") != _bf_state_key:
            st.session_state["g2_odds_h"] = round(bf_lay_default, 2) if bf_lay_default > 1 else 2.0
            st.session_state["g2_odds_d"] = 3.5
            st.session_state["g2_odds_a"] = 2.0
            st.session_state["g2_ou25_under"] = 0.0
            st.session_state["g2_ou25_over"] = 0.0
            st.session_state["g2_btts_yes"] = 0.0
            st.session_state["g2_btts_no"] = 0.0
            ...
            if bf_cs_btn:
              with st.spinner("Scraping Betfair Exchange (1X2 + BTTS + O/U)..."):
                try:
                    ...
                    _1x2_mids = get_1x2_mids(cs_data)
                    if _1x2_mids:
                        st.session_state["g2_odds_h"] = _1x2_mids[0]
                        st.session_state["g2_odds_d"] = _1x2_mids[1]
                        st.session_state["g2_odds_a"] = _1x2_mids[2]
                    _ou25 = get_ou25_both_mids(cs_data)
                    if _ou25:
                        st.session_state["g2_ou25_under"] = _ou25[0]
                        st.session_state["g2_ou25_over"] = _ou25[1]
                    _btts = get_btts_both_mids(cs_data)
                    if _btts:
        ...
        if cs_data and not cs_data.error:
            ...
            with st.expander("📡 Données brutes Betfair (back / lay / mid)", expanded=False):
                raw_rows = []
                if cs_data.match_odds_1x2_detail:
                    for name, det in cs_data.match_odds_1x2_detail.items():
                        raw_rows.append({
                            "Marché": "1X2", "Sélection": name,
                            "Back All": f"{det.back:.2f}",
                            "Lay All": f"{det.lay:.2f}",
                            "Mid": f"{det.mid_price:.3f}",
                        })
                _2way_markets = [
                    ("O/U 2.5", cs_data.ou25),
                    ("BTTS", cs_data.btts),
                ]
                for label, mkt in _2way_markets:
            ...
            if st.session_state.get("_g2_mkt_auto_key") != _mkt_state_key:
                _1x2_mids = get_1x2_mids(cs_data)
                if _1x2_mids:
                    st.session_state["g2_odds_h"] = _1x2_mids[0]
                    st.session_state["g2_odds_d"] = _1x2_mids[1]
        ...
        if not scrape_info_parts:
            st.caption("Saisissez les cotes 1X2 (mid-price), puis O/U 2.5 et BTTS pour plus de précision. Ou scrapez Betfair.")

        _home_label = g2_match.get("home", "Home")
        _away_label = g2_match.get("away", "Away")

        st.markdown("##### 🎯 1X2 — Cotes mid-price (Buchdahl)")

        bf_col1, bf_col2, bf_col3 = st.columns(3)
        with bf_col1:
            g2_odds_h = st.number_input(
                f"1X2 {_home_label}",
                min_value=1.01, max_value=100.0, step=0.01,
                key="g2_odds_h",
                help="Mid-price 1X2 du Home."
            )
        with bf_col2:
            g2_odds_d = st.number_input(
                "1X2 Draw",
                min_value=1.01, max_value=100.0, step=0.01,
                key="g2_odds_d",
                help="Mid-price 1X2 du Nul."
            )
        with bf_col3:
            g2_odds_a = st.number_input(
                f"1X2 {_away_label}",
                min_value=1.01, max_value=100.0, step=0.01,
                key="g2_odds_a",
                help="Mid-price 1X2 du Away."
            )
        ...
        with bf_col4:
            g2_ou25_under = st.number_input(
                "O/U 2.5 Under",
                min_value=0.0, max_value=50.0, step=0.01,
                key="g2_ou25_under",
                help="Mid-price Under 2.5 Goals. 0 = désactivé."
            )
        with bf_col5:
            g2_ou25_over = st.number_input(
                "O/U 2.5 Over",
                min_value=0.0, max_value=50.0, step=0.01,
                key="g2_ou25_over",
                help="Mid-price Over 2.5 Goals. 0 = désactivé."
            )

        bf_col6, bf_col7 = st.columns(2)
        with bf_col6:
            g2_btts_yes = st.number_input(
                "BTTS Yes",
                min_value=0.0, max_value=50.0, step=0.01,
                key="g2_btts_yes",
                help="Mid-price BTTS Yes. 0 = désactivé."
            )
        with bf_col7:
            g2_btts_no = st.number_input(
                "BTTS No",
                min_value=0.0, max_value=50.0, step=0.01,
                key="g2_btts_no",
                help="Mid-price BTTS No. 0 = désactivé."
            )
        ...
        if st.button("🧮 Calculer EV0", type="primary", key="g2_calc"):
            with st.spinner("Calcul en cours..."):
                g2_result = compute_g2(
                    ...
                    ou25_under=g2_ou25_under if g2_ou25_under > 1.0 else None,
...
# 📦 Export Carrière TM
# ============================================================
elif page == "📦 Export Carrière TM":
    import glob
    import subprocess
    from datetime import datetime as _dt
    from pathlib import Path

    st.header("📦 Export Carrière TM")
    st.caption(
        "Export des données carrière scrapées via l'API Transfermarkt ceapi. "
        "Trois grains disponibles par championnat : `summary` (1 ligne par joueur, "
        "totaux carrière), `career` (1 ligne par saison × compétition × club), "
        "`matches` (10 derniers matchs OU 3 derniers mois, max des deux)."
    )

    CAREER_DIR = Path(__file__).parent / "live" / "data" / "tm_career"
    SCRAP_DIR = Path(__file__).parent / "live" / "data" / "tm_scrap"

    # ── Section 1 : Fichiers téléchargeables ──────────────────────
    st.subheader("📁 Fichiers disponibles")

    if not CAREER_DIR.exists() or not list(CAREER_DIR.glob("*.csv")):
        st.info(
            "Aucun fichier carrière généré pour l'instant. "
            "Voir la section ci-dessous pour lancer un scraping."
        )
    else:
        # Groupe les fichiers par préfixe ligue (mar1, alg1, sfa1, ...)
        files = sorted(CAREER_DIR.glob("*.csv"))
        leagues_found: dict[str, list[Path]] = {}
        for fp in files:
            league_code = fp.stem.split("_")[0]
            leagues_found.setdefault(league_code, []).append(fp)

        for league, fps in leagues_found.items():
            with st.expander(f"🏆 **{league.upper()}** — {len(fps)} fichiers", expanded=True):
                for fp in fps:
                    suffix = fp.stem.replace(f"{league}_", "")
                    size_kb = fp.stat().st_size / 1024
                    mtime = _dt.fromtimestamp(fp.stat().st_mtime)
                    n_lines = 0
                    try:
                        with open(fp, encoding="utf-8") as f:
                            n_lines = sum(1 for _ in f) - 1  # -1 pour header
                    except Exception:
                        pass

                    cols = st.columns([3, 1, 1, 1, 2])
                    cols[0].markdown(f"**{suffix}**  \n`{fp.name}`")
                    cols[1].metric("Lignes", f"{n_lines:,}")
                    cols[2].metric("Taille", f"{size_kb:.1f} KB")
                    cols[3].caption(f"MAJ:  \n{mtime:%Y-%m-%d %H:%M}")
                    with cols[4]:
                        try:
                            ...
                            cols[4].error(f"Erreur lecture: {e}")
    ...
    summary_files = sorted(CAREER_DIR.glob("*_summary.csv")) if CAREER_DIR.exists() else []
    if summary_files:
        league_options = [fp.stem.replace("_summary", "") for fp in summary_files]
        selected_league = st.selectbox(
            "Championnat à prévisualiser",
            league_options,
            key="career_preview_league",
        )

        try:
            import pandas as pd
            df_sum = pd.read_csv(CAREER_DIR / f"{selected_league}_summary.csv")
            df_car_path = CAREER_DIR / f"{selected_league}_career.csv"
            df_mat_path = CAREER_DIR / f"{selected_league}_matches.csv"

            mc1, mc2, mc3, mc4 = st.columns(4)
            mc1.metric("Joueurs", f"{len(df_sum):,}")
            mc2.metric(
                "Matchs joués (médiane)",
                f"{int(df_sum['career_matches_played'].median()):,}"
                if "career_matches_played" in df_sum
                else "—",
            )
            mc3.metric(
                "Buts carrière (médiane)",
                f"{int(df_sum['career_goals'].median()):,}"
                if "career_goals" in df_sum
                else "—",
            )
            mc4.metric(
                "Saisons (médiane)",
                f"{int(df_sum['career_seasons'].median())}"
                if "career_seasons" in df_sum
                else "—",
            )

            tab_sum, tab_car, tab_mat, tab_comp = st.tabs([
                "Summary (front)",
                "Career (back)",
                "Matches récents",
                "Compétitions rencontrées",
            ])

            with tab_sum:
                st.caption("Une ligne par joueur, totaux carrière complète.")
                st.dataframe(df_sum, use_container_width=True, height=400)

            with tab_car:
                st.caption(
                    "Une ligne par (joueur, saison, compétition, club). "
                    "Inclut les matchs en sélection nationale (`is_national_team=True`)."
                )
                if df_car_path.exists():
                    df_car = pd.read_csv(df_car_path)
                    st.dataframe(df_car, use_container_width=True, height=400)
                else:
                    st.info("Fichier career absent.")

            with tab_mat:
                st.caption(
                    "10 derniers matchs OU 3 derniers mois (max des deux). "
                    "`participation_state` = état natif TM "
                    "(`played` / `injured` / `absent` / `in squad` / `not in squad`)."
                )
                if df_mat_path.exists():
                    df_mat = pd.read_csv(df_mat_path)
                    st.dataframe(df_mat, use_container_width=True, height=400)
                else:
                    st.info("Fichier matches absent.")

            with tab_comp:
                st.caption(
                    "Liste exhaustive des `competition_id` Transfermarkt rencontrés "
                    "dans la carrière des joueurs de ce championnat. "
                    "Sert de base au mapping ligue → tier + rating Buteurs Maison."
                )
                comp_path = CAREER_DIR / f"{selected_league}_competitions_seen.csv"
                if comp_path.exists():
                    df_comp = pd.read_csv(comp_path)
                    st.dataframe(df_comp, use_container_width=True, height=400)
                    csv_bytes = df_comp.to_csv(index=False).encode()
                    st.download_button(
                        label="⬇️ Télécharger la liste des compétitions",
                        data=csv_bytes,
                        file_name=comp_path.name,
                        mime="text/csv",
                        key=f"dl_comp_{selected_league}",
                    )
                else:
                    st.info("Rapport compétitions absent.")
        except Exception as e:
            st.error(f"Erreur de chargement : {e}")
    else:
        st.info("Aucun summary disponible. Lancer un scraping d'abord.")

    st.markdown("---")

    # ── Section 3 : Lancer un scraping ────────────────────────────
    st.subheader("🚀 Lancer un scraping carrière")
    st.caption(
        "Scrape la carrière des joueurs actuellement actifs (saison 25/26) "
        "d'un championnat. Utilise l'API publique Transfermarkt ceapi. "
        "Compter ~3-5 minutes pour ~500 joueurs (dépend de la latence TM)."
    )

    available_leagues = []
    if SCRAP_DIR.exists():
        available_leagues = sorted(fp.stem for fp in SCRAP_DIR.glob("*.csv"))

    if not available_leagues:
        st.warning(
            "Aucun CSV source dans `live/data/tm_scrap/`. Lancer "
            "`tm_league_scraper.py` d'abord pour produire les effectifs."
        )
    else:
        sc1, sc2, sc3 = st.columns([2, 1, 1])
        with sc1:
            league_to_scrape = st.selectbox(
                "Championnat à scraper",
                available_leagues,
                key="career_scrape_league",
            )
        with sc2:
            workers = st.number_input(
                "Workers parallèles", min_value=1, max_value=8, value=5,
                key="career_scrape_workers",
            )
        with sc3:
            test_mode = st.checkbox("Mode test (5 joueurs)", value=False, key="career_test")

        if st.button("▶️ Lancer le scraping", type="primary", key="career_run"):
            cmd = [
                "python3",
                str(Path(__file__).parent / "tm_player_career.py"),
                "--league", league_to_scrape,
                "--workers", str(workers),
            ]
            if test_mode:
                cmd += ["--test", "--test-n", "5"]

            log_path = f"/tmp/career_{league_to_scrape}.log"
            try:
                ...
                st.success(
                    f"Scraping `{league_to_scrape}` lancé en arrière-plan. "
                    f"Log : `{log_path}`. Recharge la page dans 5-10 min pour voir "
                    f"les fichiers."
                )
                st.code(" ".join(cmd))
            except Exception as e:
                st.error(f"Échec lancement : {e}")

        # Affiche le log du dernier run si présent
        log_path = f"/tmp/career_{league_to_scrape}.log"
        if Path(log_path).exists():
            with st.expander(f"📜 Log du dernier run ({league_to_scrape})"):
                try:
                    log_content = Path(log_path).read_text()
                    st.code(log_content[-3000:] if len(log_content) > 3000 else log_content)
                except Exception as e:
                    st.error(f"Lecture log impossible : {e}")
